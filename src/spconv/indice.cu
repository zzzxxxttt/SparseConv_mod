    ...
    tv::dispatch_torch<int32_t>(indicesIn.scalar_type(), [&](auto IndexValue) {
      ...
      tv::dispatch_int<2, 3, 4>(ndim, [&](auto I) {
        ...
        if (useHash)
        {
          ...
          bool res = table.Build(numActIn,
                                 reinterpret_cast<unsigned *>(d_keyvalues),
                                 reinterpret_cast<unsigned *>(d_values));
          cudaFree(d_keyvalues);
          TV_CHECK_CUDA_ERR_V2("cudaFree failed");
          if (!res)
          ...
          tv::DispatchInt<max_kernel_vol_t>()(kernelVolume, std::less_equal<int>(), [&](auto I2) {
            constexpr int MaxKernelVolume = decltype(I2)::value;
            getSubMIndicePairsHashKernel_mod<Index, NDim, MaxKernelVolume>
                <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                    tv::torch2tv<Index>(indicesIn),
                    tv::torch2tv<Index>(indicePairs),
                    tv::torch2tv<Index>(indiceNum),
        ...
        else
        {
          ...
          if (dilation_one && (NDim == 2 || NDim == 3))
          {
            ...
            if (NDim == 2)
            {
              tv::SimpleVector<Index, 2> ou_(outSpatialShape.begin(), outSpatialShape.end());
              tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[0], [&](auto K0C) {
                tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[1], [&](auto K1C) {
                  constexpr int K0 = decltype(K0C)::value;
                  constexpr int K1 = decltype(K1C)::value;
                  found = true;
                  getSubMIndicePairsKernel2_mod<Index, IndexGrid, K0, K1>
                  <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                      tv::torch2tv<Index>(indicesIn),
                      tv::torch2tv<IndexGrid>(gridsOut),
                      tv::torch2tv<Index>(indicePairs),
              ...
              tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[0], [&](auto K0C) {
                tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[1], [&](auto K1C) {
                  tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[2], [&](auto K2C) {
                    ...
                    constexpr int K2 = decltype(K2C)::value;
                    found = true;
                    getSubMIndicePairsKernel3_mod<Index, IndexGrid, K0, K1, K2>
                    <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                        tv::torch2tv<Index>(indicesIn),
                        tv::torch2tv<IndexGrid>(gridsOut),
                        tv::torch2tv<Index>(indicePairs),
          ...
          if (!found)
          {
            tv::DispatchInt<max_kernel_vol_t>()(ndim, std::less_equal<int>(), [&](auto I2) {
              constexpr int MaxKernelVolume = decltype(I2)::value;
              getSubMIndicePairsKernel_mod<Index, IndexGrid, NDim, MaxKernelVolume>
              <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                  tv::torch2tv<Index>(indicesIn),
                  tv::torch2tv<IndexGrid>(gridsOut),
                  tv::torch2tv<Index>(indicePairs),
    ...
    tv::dispatch_torch<int32_t>(indicesIn.scalar_type(), [&](auto IndexValue) {
      ...
      tv::dispatch_int<2, 3, 4>(ndim, [&](auto I) {
        ...
        if (useHash) {
          ...
          tv::DispatchInt<max_kernel_vol_t>()(kernelVolume, std::less_equal<int>(), [&](auto I2) {
            constexpr int MaxKernelVolume = decltype(I2)::value;
            getSubMIndicePairsHashKernel_mod21<Index, NDim, MaxKernelVolume>
            <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                tv::torch2tv<Index>(indicesIn),
                tv::torch2tv<Index>(indiceNum),
                ks, st, pa, di, ou,
            ...
            int MaxSize_2nd = indiceNum.max().item().toInt();
            indicePairs = torch::full({2, kernelVolume / 2, MaxSize_2nd}, -1,
                                      torch::dtype(torch::kInt32).device(torch::kCUDA));
            indiceNum.fill_(0);

            getSubMIndicePairsHashKernel_mod22<Index, NDim, MaxKernelVolume>
            <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                tv::torch2tv<Index>(indicesIn),
                tv::torch2tv<IndexGrid>(indicePairs),
                tv::torch2tv<Index>(indiceNum),
                ks, st, pa, di, ou,
                tableSize,
                tableData,
            ...
            if (NDim == 2) {
              ...
              tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[0], [&](auto K0C) {
                tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[1], [&](auto K1C) {
                  ...
                  constexpr int K1 = decltype(K1C)::value;
                  found = true;
                  getSubMIndicePairsKernel2_mod21<Index, IndexGrid, K0, K1>
                  <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                      tv::torch2tv<Index>(indicesIn),
                      tv::torch2tv<IndexGrid>(gridsOut),
                      tv::torch2tv<Index>(indiceNum),
                  ...
                  getSubMIndicePairsKernel2_mod22<Index, IndexGrid, K0, K1>
                  <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                      tv::torch2tv<Index>(indicesIn),
                      tv::torch2tv<IndexGrid>(gridsOut),
                      tv::torch2tv<Index>(indicePairs),
              ...
              tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[0], [&](auto K0C) {
                tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[1], [&](auto K1C) {
                  tv::dispatch_int_noexcept<1, 3, 5>(kernelSize[2], [&](auto K2C) {
                    ...
                    getSubMIndicePairsKernel3_mod21<Index, IndexGrid, K0, K1, K2>
                    <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                        tv::torch2tv<Index>(indicesIn),
                        tv::torch2tv<IndexGrid>(gridsOut),
                        tv::torch2tv<Index>(indiceNum),
                    ...
                    getSubMIndicePairsKernel3_mod22<Index, IndexGrid, K0, K1, K2>
                    <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                        tv::torch2tv<Index>(indicesIn),
                        tv::torch2tv<IndexGrid>(gridsOut),
                        tv::torch2tv<Index>(indicePairs),
          ...
          if (!found) {
            tv::DispatchInt<max_kernel_vol_t>()(ndim, std::less_equal<int>(), [&](auto I2) {
              constexpr int MaxKernelVolume = decltype(I2)::value;
              getSubMIndicePairsKernel_mod21<Index, IndexGrid, NDim, MaxKernelVolume>
              <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                  tv::torch2tv<Index>(indicesIn),
                  tv::torch2tv<IndexGrid>(gridsOut),
                  tv::torch2tv<Index>(indiceNum),
              ...
              getSubMIndicePairsKernel_mod22<Index, IndexGrid, NDim, MaxKernelVolume>
              <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                  tv::torch2tv<Index>(indicesIn),
                  tv::torch2tv<IndexGrid>(gridsOut),
                  tv::torch2tv<Index>(indicePairs),
