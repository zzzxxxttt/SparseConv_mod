...
int create_submconv_indice_pair_cuda(
  ...
  int create_submconv_indice_pair_cuda_mod(torch::Tensor indicesIn,              // [N_points, Ndim]
                                          torch::Tensor gridsOut,               // [batchSize] if use hash else [batchSize, outputVolume]
                                          ...
                                          std::vector<int64_t> outSpatialShape, // [16, 16, 16]
                                          bool transpose,
                                          bool resetGrid,
                                          bool useHash)
  {
        ...
        if (useHash)
        {
          ...
          prepareSubMHashKernel<Index, NDim>
              <<<tv::cuda::getBlocks(numActIn), tv::cuda::CUDA_NUM_THREADS, 0, stream>>>(
                  tv::torch2tv<Index>(indicesIn), // [numAct, Ndim]
                  d_keyvalues,                    // [kernelVolume*2]
                  d_values,                       // [kernelVolume]
                  ou);                            // [16, 16, 16]
          TV_CHECK_CUDA_ERR_V2("prepareSubMHashKernel failed");
