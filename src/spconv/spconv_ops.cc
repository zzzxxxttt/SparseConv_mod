...
getIndicePairs(torch::Tensor indices, int64_t batchSize,
                                               ...
                                               int64_t _useHash)
  {
    ...
    if (subM)
    {
      ...
      {
        numActOut = create_submconv_indice_pair_cuda_mod(indices, gridOut, indicePairs, indiceNum,
                                                        kernelSize, stride, padding, dilation,
                                                        outSpatialShape, transpose, false, useHash);
        if (numActOut == -1)
        {
    ...
    {
      TV_THROW_INVALID_ARG("v2 only support subM conv yet!");
    }
  }
...
torch::Tensor indiceConv(torch::Tensor features, torch::Tensor filters,
                              ...
                              int64_t algo)
  {
    ...
    case kBatch:
    {
      if (kernelVolume != 1)
      {
        TV_THROW_RT_ERR("v2 does not support batch gemm yet!");
      }
      else
      {
        break;
    ...
    {
      TV_THROW_RT_ERR("v2 only support subM conv yet!");
      indicePairMaxSize = *std::max_element(indicePairNumCpu.data_ptr<int>(),
                                            indicePairNumCpu.data_ptr<int>() + kernelVolume);
    }
    ...
    for (int i = 0; i < kernelVolume; ++i)
    {
      ...
      {
        nHot = indicePairNumCpu.data_ptr<int>()[kernelVolume - i - 1];
      }
      ...
      if (device == torch::kCPU)
      {
        ...
        {
          sparse_gather_cpu(inputBuffer, features, indicePairs[!inverse][kernelVolume - i - 1], nHot);
        }
      }
#ifdef TV_CUDA
      else if (device == torch::kCUDA)
      {
        ...
        {
          sparse_gather_cuda(inputBuffer, features, indicePairs[!inverse][kernelVolume - i - 1], nHot);
        }
      ...
      // totalGatherTime += timer.report() / 1000.0;
      // multiply the gathered features with the ith kernel kernel
      torch::mm_out(outputBufferBlob, inputBufferBlob, filters[i]);
      // totalGEMMTime += timer.report() / 1000.0;
      ...
      if (device == torch::kCPU)
      {
        ...
        {
          sparse_scatter_add_cpu(outputBuffer, output, indicePairs[inverse][kernelVolume - i - 1], nHot);
        }
      }
#ifdef TV_CUDA
      else if (device == torch::kCUDA)
      {
        ...
        {
          sparse_scatter_add_cuda(outputBuffer, output, indicePairs[inverse][kernelVolume - i - 1], nHot);
        }
      }
#endif
      else
...
indiceConvBackward(torch::Tensor features, torch::Tensor filters,
                                                   ...
                                                   int64_t algo)
  {
    ...
    case kBatch:
    {
      if (kernelVolume != 1)
      {
        TV_THROW_RT_ERR("v2 does not support batch backward yet!");
      }
      else
      {
        break;
    ...
    for (int i = 0; i < kernelVolume; ++i)
    {
      ...
      if (device == torch::kCPU)
      {
        ...
        {
          sparse_gather_cpu(inputBuffer, features, indicePairs[!inverse][kernelVolume - i - 1], nHot);
          sparse_gather_cpu(outputBuffer, outGrad, indicePairs[inverse][kernelVolume - i - 1], nHot);
        }
      }
#ifdef TV_CUDA
      else if (device == torch::kCUDA)
      {
        ...
        {
          sparse_gather_cuda(inputBuffer, features, indicePairs[!inverse][kernelVolume - i - 1], nHot);
          sparse_gather_cuda(outputBuffer, outGrad, indicePairs[inverse][kernelVolume - i - 1], nHot);
        }
      }
#endif
      else
      ...
      if (device == torch::kCPU)
      {
        ...
        {
          sparse_scatter_add_cpu(inputBuffer, inputGrad, indicePairs[!inverse][kernelVolume - i - 1], nHot);
        }
      }
#ifdef TV_CUDA
      else if (device == torch::kCUDA)
      {
        ...
        {
          sparse_scatter_add_cuda(inputBuffer, inputGrad, indicePairs[!inverse][kernelVolume - i - 1], nHot);
        }
      }
#endif
      else
