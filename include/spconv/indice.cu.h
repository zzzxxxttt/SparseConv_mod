...
#pragma unroll
  ...
  template<typename Index, typename IndexGrid, unsigned NDim,
      int KernelMaxVolume = 256>
  __global__ void getSubMIndicePairsKernel_mod(tv::TensorView<const Index> indicesIn,
...
#pragma unroll
    ...
    Index index = 0;
    for (int ix : tv::KernelLoopX<int>(numActIn)) {
      numValidPoints = getValidOutPos<Index, NDim>(indicesIn.data() + ix * (NDim + 1) + 1,
                                                   kernelSize.data(),
                                                   stride.data(),
                                                   padding.data(),
                                                   dilation.data(),
      ...
      for (int i = 0; i < numValidPoints; ++i) {
        pointPtr = validPoints + i * (NDim + 1);
        auto offset = pointPtr[NDim];
        index = tv::ArrayIndexRowMajor<NDim, NDim>::runPtrs(pointPtr, outSpatialShape.data(), 0) +
                spatialVolume * indicesIn(ix, 0);
        if (gridsOut[index] > -1 && offset < halfKernelVolume) {
          Index oldNum = atomicAdd(indiceNum.data() + offset, Index(1));
          indicePairs(1, offset, oldNum) = gridsOut[index];
          indicePairs(0, offset, oldNum) = ix;
        }
  ...
  template<typename Index, typename IndexGrid, unsigned NDim,
      int KernelMaxVolume = 256>
  __global__ void getSubMIndicePairsKernel_mod21(tv::TensorView<const Index> indicesIn,
                                                 tv::TensorView<IndexGrid> gridsOut,
                                                 tv::TensorView<Index> indiceNum,
                                                 const tv::SimpleVector<Index, NDim> kernelSize,
                                                 const tv::SimpleVector<Index, NDim> stride,
...
//          indicePairs(0, offset, oldNum) = ix;
  ...
  template<typename Index, typename IndexGrid, unsigned NDim,
      int KernelMaxVolume = 256>
  __global__ void getSubMIndicePairsKernel_mod22(tv::TensorView<const Index> indicesIn,
                                                 tv::TensorView<IndexGrid> gridsOut,
                                                 tv::TensorView<Index> indicePairs,
                                                 tv::TensorView<Index> indiceNum,
                                                 const tv::SimpleVector<Index, NDim> kernelSize,
...
#pragma unroll
  ...
  template<typename Index, typename IndexGrid, unsigned K0, unsigned K1,
      unsigned K2>
  __global__ void getSubMIndicePairsKernel3_mod(tv::TensorView<const Index> indicesIn,
                                                tv::TensorView<IndexGrid> gridsOut,
                                                tv::TensorView<Index> indicePairs,
                                                tv::TensorView<Index> indiceNum,
...
//            }
  ...
  template<typename Index, typename IndexGrid, unsigned K0, unsigned K1,
      unsigned K2>
  __global__ void getSubMIndicePairsKernel3_mod21(tv::TensorView<const Index> indicesIn,
                                                  tv::TensorView<IndexGrid> gridsOut,
                                                  tv::TensorView<Index> indiceNum,
                                                  const tv::SimpleVector<Index, 3> outSpatialShape,
                                                  Index spatialVolume) {
...
//          }
  ...
  template<typename Index, typename IndexGrid, unsigned K0, unsigned K1,
      unsigned K2>
  __global__ void getSubMIndicePairsKernel3_mod22(tv::TensorView<const Index> indicesIn,
                                                  tv::TensorView<IndexGrid> gridsOut,
                                                  tv::TensorView<Index> indicePairs,
                                                  tv::TensorView<Index> indiceNum,
                                                  const tv::SimpleVector<Index, 3> outSpatialShape,
...
#pragma unroll
  ...
  template<typename Index, typename IndexGrid, unsigned K0, unsigned K1>
  __global__ void getSubMIndicePairsKernel2_mod(tv::TensorView<const Index> indicesIn,
                                                tv::TensorView<IndexGrid> gridsOut,
                                                tv::TensorView<Index> indicePairs,
                                                tv::TensorView<Index> indiceNum,
...
//          }
  ...
  template<typename Index, typename IndexGrid, unsigned K0, unsigned K1>
  __global__ void getSubMIndicePairsKernel2_mod21(tv::TensorView<const Index> indicesIn,
                                                  tv::TensorView<IndexGrid> gridsOut,
                                                  tv::TensorView<Index> indiceNum,
                                                  const tv::SimpleVector<Index, 2> outSpatialShape,
                                                  Index spatialVolume) {
...
//          }
  ...
  template<typename Index, typename IndexGrid, unsigned K0, unsigned K1>
  __global__ void getSubMIndicePairsKernel2_mod22(tv::TensorView<const Index> indicesIn,
                                                  tv::TensorView<IndexGrid> gridsOut,
                                                  tv::TensorView<Index> indicePairs,
                                                  tv::TensorView<Index> indiceNum,
                                                  const tv::SimpleVector<Index, 2> outSpatialShape,
...
//          }
  ...
  template<typename Index, unsigned NDim, int KernelMaxVolume = 256, unsigned kNumHashFunctions = 4>
...
#pragma unroll
  ...
  template<typename Index, unsigned NDim, int KernelMaxVolume = 256, unsigned kNumHashFunctions = 4>
  __global__ void getSubMIndicePairsHashKernel_mod(
...
#pragma unroll
    ...
    for (int ix : tv::KernelLoopX<int>(numActIn)) {
      ...
      for (int i = 0; i < numValidPoints; ++i) {
        ...
        auto val = cuhash::retrieve((unsigned) (index),
                                    ...
                                    stash_constants,
                                    stash_count);
        if (val != cuhash::kNotFound && offset < halfKernelVolume) {
          Index oldNum = atomicAdd(indiceNum.data() + offset, Index(1));
          indicePairs(1, offset, oldNum) = val;
          indicePairs(0, offset, oldNum) = ix;
        }
  ...
  template<typename Index, unsigned NDim, int KernelMaxVolume = 256, unsigned kNumHashFunctions = 4>
  __global__ void getSubMIndicePairsHashKernel_mod21(
      tv::TensorView<const Index> indicesIn,
      tv::TensorView<Index> indiceNum,
      const tv::SimpleVector<Index, NDim> kernelSize,
      const tv::SimpleVector<Index, NDim> stride,
...
#pragma unroll
  ...
  template<typename Index, unsigned NDim, int KernelMaxVolume = 256, unsigned kNumHashFunctions = 4>
  __global__ void getSubMIndicePairsHashKernel_mod22(
      tv::TensorView<const Index> indicesIn,
      tv::TensorView<Index> indicePairs,
      tv::TensorView<Index> indiceNum,
      const tv::SimpleVector<Index, NDim> kernelSize,
