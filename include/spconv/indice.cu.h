...
template <typename Index, unsigned NDim, int KernelMaxVolume = 256, unsigned kNumHashFunctions = 4>
  __global__ void getSubMIndicePairsHashKernel_mod(
      tv::TensorView<const Index> indicesIn,               // [numAct, Ndim]
      tv::TensorView<Index> indicePairs,                   // [2, kernelVolume/2+1, numAct]
      tv::TensorView<Index> indiceNum,                     // [kernelVolume]
      const tv::SimpleVector<Index, NDim> kernelSize,      // [3, 3, 3]
      const tv::SimpleVector<Index, NDim> stride,          // [1, 1, 1]
      const tv::SimpleVector<Index, NDim> padding,         // [1, 1, 1]
      const tv::SimpleVector<Index, NDim> dilation,        // [1, 1, 1]
      const tv::SimpleVector<Index, NDim> outSpatialShape, // [16, 16, 16]
      unsigned table_size,
...
#pragma unroll
    ...
    Index *pointPtr = nullptr;
    Index index = 0;
    // using numAct threads to find the valid output points of input points
    // each thread is responsible for one input point
    for (int ix : tv::KernelLoopX<int>(numActIn))
    {
